...
import { Timeout, Cron, NestSchedule } from 'nest-schedule';
...
@Injectable()
export default class MantaService extends NestSchedule {
  constructor(
    @InjectEntityManager('manta')
    private mantaSlpDB: EntityManager,
  ...
  async mantaScan() {
    ...
    const wsProvider = new WsProvider(
      // 'wss://manta.api.onfinality.io/public-ws',
      'wss://ws.manta.systems',
    );
    const api = await ApiPromise.create({ provider: wsProvider });

    const stakingEras = await this.mantaSlpDB
    ...
    await this.markDB
      ...
      .execute();
    await this.markDB
      .getRepository(MantaValidatorEra)
      .createQueryBuilder()
      .where('CAST(era AS INT) < :era', {
        era: Number(stakingEras[0].era_index) - 100,
      })
    ...
    const mantaEra = await this.markDB
      .getRepository(MantaEra)
      .createQueryBuilder()
      .orderBy('era', 'DESC')
      .getMany();
    const eras = stakingEras.filter(
      (item) => !mantaEra.map((item) => item.era).includes(item.era_index),
    );
    ...
    for (let i = 0; i < eras.length; i++) {
      ...
      const validatorsWaitEraData = await Promise.all(
        waitValidators.map(async (validator) => {
          ...
          const rank = new BigNumber(average_reward_points)
            ...
            .multipliedBy(0.9)
            .div(
              new BigNumber(currentMantaEra.min_bond)
      ...
      await this.markDB.getRepository(MantaEra).save(currentMantaEra);

      console.log(`manta ${era} scan finished!`);
    }

    console.log('manta staking scan finished!');
  }
}
