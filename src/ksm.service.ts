...
@Injectable()
export default class KsmService extends NestSchedule {
  constructor(
    @InjectEntityManager('kusama')
    private kusamaSlpDB: EntityManager,
  ...
  async kusamaScan() {
    ...
    await this.kusamaMarkDB
      .getRepository(KusamaEra)
      .createQueryBuilder()
      .where('CAST(era AS INT) < :era', {
        era: Number(stakingEras[0].era_index) - 56,
      })
      .delete()
      .execute();
    await this.kusamaMarkDB
    ...
    for (let i = 0; i < eras.length; i++) {
      ...
      await this.kusamaMarkDB.getRepository(KusamaEra).save(kusamaEra);

      console.log(`${era} scan finished!`);
    }

    console.log('kusama staking scan finished!');
  }
}
