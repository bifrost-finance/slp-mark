...
@Injectable()
export default class MovrService extends NestSchedule {
  constructor(
    @InjectEntityManager('moonriver')
    private moonriverSlpDB: EntityManager,
  ...
  async moonriverScan() {
    ...
    const stakingEras = await this.moonriverSlpDB
    ...
    const moonriverEra = await this.markDB
      .getRepository(MoonriverEra)
      .createQueryBuilder()
      .orderBy('era', 'DESC')
      .getMany();
    const eras = stakingEras.filter(
      (item) => !moonriverEra.map((item) => item.era).includes(item.era_index),
    );
    ...
    console.log('moonriver staking scan finished!');
  }
}
